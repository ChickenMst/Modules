...
modules.services.player.players = {} -- table of players
...
modules.libraries.callbacks:connect("onPlayerJoin", function(steam_id, name, peer_id, is_admin, is_auth)
    ...
    local player = modules.classes.player:create(peer_id, steam_id, name, is_admin, is_auth)
    if not player then
...
function modules.services.player:getPlayer(steam_id)
    ...
    local player = modules.services.player.players[tostring(steam_id)]
    if not player then
        modules.libraries.logging:warning("services.player:getPlayer", "Player not found with steam_id: " .. steam_id)
        return nil
    end
    ...
    if service.players then
        for _, playerData in pairs(service.players) do
            ...
            if not player then
                ...
                modules.libraries.logging:debug("services.player:_load", "Loaded player: " .. playerData.name .. " with steam_id: " .. playerData.steam_id)
            end
        end
    end
end
